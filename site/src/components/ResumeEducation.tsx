        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6" staggerDelay={0.1}>
          {educationList.map((item) => (
            <StaggerItem key={item.institution}>
              <div className="resume-education-card rounded-2xl bg-surface border border-border p-5 sm:p-6 shadow-sm hover:border-forest/30 transition-all duration-200 h-full flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-sm font-medium text-forest font-sans mt-0.5">
                        {item.degree}
                      </p>
                    </div>

                    {item.honors && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-mono font-medium bg-forest-light text-forest border border-forest/20">
                        <Award size={12} />
                        {item.honors}
                      </span>
                    )}
