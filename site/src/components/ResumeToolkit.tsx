      ...
      <div className="mx-auto max-w-4xl">
        <ScrollReveal>
          <div className="flex items-center gap-2.5 mb-3">
            <span className="p-1.5 rounded-lg bg-sprout-light text-forest">
              <Wrench size={16} />
            </span>
          ...
          <div className="h-0.5 w-12 bg-sprout/50 rounded-full mb-6" />
        </ScrollReveal>
          ...
          {technicalToolkit.map((category) => {
            ...
            return (
              <StaggerItem key={category.title}>
                <div className="rounded-2xl bg-surface border border-border p-5 sm:p-6 shadow-sm hover:border-forest/30 transition-all duration-200 h-full flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      <div className="p-1.5 rounded-lg bg-surface-alt text-forest border border-border/80">
                        <Icon size={15} />
                      </div>
                      ...
                      {category.skills.map((skill) => (
                        <span
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-mono transition-all duration-200 border ${
                            skill.featured
                              ? "bg-forest-light text-forest border-forest/20 font-medium"
                              : "bg-surface-alt text-muted hover:text-foreground border-border"
                          }`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${
                              skill.featured ? "bg-forest" : "bg-muted/60"
                            }`}
                          />
                          {skill.name}
                        </span>
